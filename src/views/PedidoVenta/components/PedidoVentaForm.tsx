              ...
              const tienePromo = promociones.find((promo) =>
                promo.promocionDetalle?.some((detalle) => {
                  if (detalle.articuloInsumo?.id && card.articuloInsumo?.id)
                    return (
                      detalle.articuloInsumo?.id === card.articuloInsumo?.id
                    );
                  else
                    return (
              ...
              <Grid size={5} sx={{ marginBottom: 2 }}>
                <Autocomplete
                  ...
                  onChange={(_, newValue) => {
                    setPedidoVenta((prev) =>
                      prev
                        ? { ...prev, tipoEnvio: newValue as TipoEnvio }
                        : prev
                    );
                  }}
                  renderInput={(params) => (
                  ...
                  <Box display={"flex"} justifyContent={"space-between"}>
                    <Table>
                      <TableHead>
                        <TableRow>
                          <TableCell>
                            <Typography variant="h6">Articulo</Typography>
                          </TableCell>
                          <TableCell>
                      ...
                      <TableBody>
                        {pedidoVenta.pedidoVentaDetalle.map(
                          (detalle, index) => (
                            <TableRow key={index}>
                              <TableCell>
                              ...
                              <TableCell>
                                <Typography variant="h6">
                                  $
                                  {detalle.articuloInsumo
                                    ? detalle.articuloInsumo.precioVenta.toFixed(
                                        2
                                      )
